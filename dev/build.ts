...
// todo buildWorkers could be done with one esbuild.build and splitting: false
async function buildWorker(urlPath: string, src: string): Promise<string> {
    ...
    const buildResult = await esbuild.build({
        ...esbuildBuildOptsForWorker(true),
        entryPoints: [src],
        entryNames: naming,
        metafile: true,
...
export async function performBuild(): Promise<{
    ...
    // await rewriteHashedWorkerURLs(buildUrls, workerHashing)
