...
export const ChooseProject: FC<ChooseProjectProps> = ({ ghToken, onProjectChoice }) => {
  ...
  return <div>
    ...
    {username && <p>
      {username} / <input type="text" disabled={clicked} onChange={(e) => onChange(e.target.value)} value={repo} />
      {error === 'not-a-repo' && <p style={{ color: 'orangered' }}>That ain't no repo, foo!</p>}
    </p>}
    <div>
      <button disabled={clicked || !repo.length} onClick={onClick}>Let's fucking rock!</button>
    </div>
  </div>
}
