...
import type { RepoDirListing, RepoFile, RepoSources } from '../workspace/RepoSources.ts'
...
export const DirIndex: FC<DirIndexProps> = ({ dirpath, repo, sources }) => {
  ...
  if (listing.status === 'loading' && listing.cached?.length) {
    ...
    return <div>
      {listing.cached.map(name => <div key={name} className="file">{name}</div>)}
    </div>
  }
      ...
      {listing.files.map(source => {
        switch (source.type) {
          ...
          case 'file-cat':
            ...
            const onClick = isOpen ? undefined : () => sources.openFile = source
            return <div key={source.name} className={className} onClick={onClick}>
              {source.name}
            </div>
        }
      })}
